...
void ChessRules::PlayMove(Move imove)
{
	...
	m_halfMoveHistory.push(half_move_clock);
...
void thc::ChessRules::UnplayMove(Move imove)
{
	//go back to previous in history
	history[--history_idx] = Move();


	//rewind half move clock
	if (white)
		full_move_count--;

	//rewind half move clock
	if (!m_halfMoveHistory.empty())
	{
		half_move_clock = m_halfMoveHistory.top();
		m_halfMoveHistory.pop();
	}
